...
impl Options {
    pub fn validate(&self) -> Result<(), Error> {
        if self.host.is_empty() || self.host.len() > 255
            || !self.host.bytes().all(|b| b.is_ascii_alphanumeric() || b".-:_".contains(&b))
        {
            return Err(Error::new(Kind::Configuration, "host must be a DNS name or an unbracketed IP address, not an SSH alias/config fragment"));
        }
        if self.port == 0 || self.user.is_empty() || self.user.len() > 256
            || self.user.chars().any(|c| c.is_control() || c.is_whitespace())
        {
            return Err(Error::new(Kind::Configuration, "invalid SSH user or port"));
        }
        if self.timeout < time::Duration::from_millis(1) || self.timeout > time::Duration::from_secs(300) {
            return Err(Error::new(Kind::Configuration, "SSH timeout must be between 1 ms and 300 s"));
        }
        if self.known_hosts.as_os_str().is_empty() {
            return Err(Error::new(Kind::Configuration, "an explicit known-hosts file is required"));
        }
        Ok(())
    }
}
...
impl Connection {
    pub fn connect(options: &Options) -> Result<Self, Error> {
        options.validate()?;
        let file = fs::File::open(&options.known_hosts)
        ...
        if bytes.len() as u64 > MAX_KNOWN_HOSTS_BYTES {
            return Err(Error::new(Kind::Configuration, "known-hosts file exceeds 4 MiB"));
        }
        ...
        socket.set_nodelay(true).map_err(|e| Error::new(Kind::Transport, e))?;
        socket.set_nonblocking(true).map_err(|e| Error::new(Kind::Transport, e))?;
        let mut session = ssh2::Session::new().map_err(|e| Error::new(Kind::Transport, e))?;
        session.set_tcp_stream(socket.try_clone().map_err(|e| Error::new(Kind::Transport, e))?);
        session.set_timeout(options.timeout.as_millis() as u32);
        session.handshake().map_err(|e| Error::new(Kind::Transport, e))?;
        ...
        let fingerprint = format!("SHA256:{}", base64::Engine::encode(
            &base64::engine::general_purpose::STANDARD_NO_PAD, hash));
        verify_known_hosts(&session, text, &options.host, options.port, key)
            .map_err(|mut error| { error.detail.push_str(&format!("; presented {fingerprint}")); error })?;
        ...
        if !session.authenticated() {
            return Err(Error::new(Kind::Authentication, "authentication was not completed"));
        }
        ...
        unsafe { poller.add(&socket, polling::Event::readable(0)) }
            .map_err(|e| Error::new(Kind::Transport, e))?;
        Ok(Self {
            session, socket, poller, events: polling::Events::new(),
            timeout: options.timeout, fingerprint,
        })
    }
    ...
    pub fn exec(mut self, command: &str) -> Result<Channel, Error> {
        ...
        Ok(Channel { inner, connection: self })
    }

    fn retry<T>(&mut self, deadline: time::Instant,
        mut operation: impl FnMut(&mut ssh2::Session) -> Result<T, ssh2::Error>) -> Result<T, Error>
    {
        loop {
            remaining(deadline)?;
...
impl Channel {
    pub fn timeout(&self) -> time::Duration { self.connection.timeout }
    pub fn fingerprint(&self) -> &str { self.connection.fingerprint() }
...
impl io::Write for Channel {
    ...
    fn flush(&mut self) -> io::Result<()> { io::Write::flush(&mut self.inner) }
}
...
fn validate_known_hosts(text: &str) -> Result<(), Error> {
    for (index, line) in text.lines().enumerate() {
        ...
        if hosts.starts_with('@') || hosts.contains(['*', '?', '!']) {
            return Err(Error::new(Kind::Configuration, format!(
                "known-hosts line {} uses unsupported markers/patterns; refusing to ignore trust policy", index + 1)));
        }
    }
    Ok(())
}

fn verify_known_hosts(session: &ssh2::Session, text: &str, host: &str,
    port: u16, key: &[u8]) -> Result<(), Error>
{
    validate_known_hosts(text)?;
    let mut hosts = session.known_hosts().map_err(|e| Error::new(Kind::Configuration, e))?;
    ...
    let lookup = if port == 22 { host.to_owned() } else { format!("[{host}]:{port}") };
    match hosts.check(&lookup, key) {
        ssh2::CheckResult::Match => Ok(()),
        ssh2::CheckResult::NotFound => Err(Error::new(Kind::UnknownHostKey, "host key is not trusted; authenticate it out of band before adding it to known_hosts")),
        ssh2::CheckResult::Mismatch => Err(Error::new(Kind::ChangedHostKey, "host key differs from known_hosts; refusing authentication")),
        ssh2::CheckResult::Failure => Err(Error::new(Kind::Configuration, "known-hosts verification failed")),
    }
}
...
mod tests {
    ...
    fn host_keys_are_exact_and_port_scoped() {
        ...
        assert_eq!(verify_known_hosts(&session, &text, "example.test", 22, &key).unwrap_err().kind, Kind::UnknownHostKey);
        assert_eq!(verify_known_hosts(&session, &text, "example.test", 2223, &key).unwrap_err().kind, Kind::UnknownHostKey);
        assert_eq!(verify_known_hosts(&session, &text, "other.test", 2222, &key).unwrap_err().kind, Kind::UnknownHostKey);
        let mut changed = key.clone();
        changed[20] ^= 1;
        assert_eq!(verify_known_hosts(&session, &text, "example.test", 2222, &changed).unwrap_err().kind, Kind::ChangedHostKey);
        let bare = format!("example.test ssh-ed25519 {encoded}\n");
        assert_eq!(verify_known_hosts(&session, &bare, "example.test", 2222, &key).unwrap_err().kind, Kind::UnknownHostKey);
    }

    #[test]
    fn unsupported_trust_policy_is_not_ignored() {
        for line in ["@revoked host ssh-ed25519 AAAA", "@cert-authority host key", "*.test key", "!bad.test,good.test key"] {
            assert_eq!(validate_known_hosts(line).unwrap_err().kind, Kind::Configuration);
        }
