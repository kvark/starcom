...
fn run() -> anyhow::Result<()> {
    ...
    while let Some(arg) = args.next() {
        ...
        match arg {
            ...
            "--host" | "--user" | "--session" | "--known-hosts" | "--identity" |
            "--port" | "--socket" | "--history" | "--timeout" => {
                let value = args.next().with_context(|| format!("{arg} needs a value"))?;
                anyhow::ensure!(values.insert(arg.to_owned(), value).is_none(), "duplicate option {arg}");
            }
            _ => anyhow::bail!("unknown option {arg:?}; use --help"),
        }
    }
    if values.is_empty() && !agent { print!("{HELP}"); return Ok(()); }
    let host = text(&mut values, "--host")?;
    let user = text(&mut values, "--user")?;
    let session = core::SessionName::new(text(&mut values, "--session")?)?;
    let known_hosts = path::PathBuf::from(values.remove("--known-hosts").context("--known-hosts is required")?);
    let identity = values.remove("--identity").map(path::PathBuf::from);
    anyhow::ensure!(agent != identity.is_some(), "choose exactly one of --identity and --agent");
    ...
    let socket = values.remove("--socket").map(|value| value.into_string()
        .map_err(|_| anyhow::anyhow!("remote socket path must be UTF-8"))).transpose()?;
    anyhow::ensure!(history <= inspect::MAX_HISTORY_LINES, "--history exceeds {}", inspect::MAX_HISTORY_LINES);
    let options = ssh::Options { host, port, user, known_hosts, authentication,
    ...
    println!("Starcom inspection: tmux {:?}, session {}, {} panes, verified {}",
        observation.tmux_version, observation.session, observation.captures.len(), observation.fingerprint);
    ...
    for capture in observation.captures {
        println!("Pane {} window {}: {}x{} at {},{}; cursor {},{}; alternate={}; retained history={}/{}",
            capture.pane.id, capture.pane.window, capture.pane.size.columns(), capture.pane.size.rows(),
            capture.pane.left, capture.pane.top, capture.pane.cursor_x, capture.pane.cursor_y,
            capture.pane.alternate_screen, capture.pane.history_size, capture.pane.history_limit);
        for (row, text) in capture.escaped_rows.iter().enumerate() {
            if !text.is_empty() { println!("  {row:>4}: {text:?}"); }
        }
    }
    Ok(())
}

fn text(values: &mut collections::BTreeMap<String, ffi::OsString>, key: &str) -> anyhow::Result<String> {
    values.remove(key).with_context(|| format!("{key} is required"))?
        .into_string().map_err(|_| anyhow::anyhow!("{key} must be UTF-8"))
}

fn optional_number<T: std::str::FromStr>(values: &mut collections::BTreeMap<String, ffi::OsString>,
    key: &str, default: T) -> anyhow::Result<T>
{
    match values.remove(key) {
        Some(value) => value.to_str().with_context(|| format!("{key} must be UTF-8"))?
            .parse().map_err(|_| anyhow::anyhow!("invalid number for {key}")),
        None => Ok(default),
    }
}
