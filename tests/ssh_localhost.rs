...
fn options() -> ssh::Options {
    ssh::Options {
        ...
        port: fs::read_to_string(root().join("port")).unwrap().trim().parse().unwrap(),
...
fn tmux(args: &[&str]) -> String {
    let result = process::Command::new("timeout").args(["5s", "tmux", "-S"])
        .arg(root().join("tmux.sock")).args(args).output().unwrap();
...
fn inspect_existing_primary_and_alternate_screens_without_mutation() {
    let before = tmux(&["list-panes", "-s", "-t", "starcom", "-F", "#{pane_id} #{pane_width} #{pane_height} #{pane_pid}"]);
    ...
    assert!(observed.captures.iter().any(|capture| capture.pane.alternate_screen));
    assert!(observed.fingerprint.starts_with("SHA256:"));
    let rows: Vec<_> = observed.captures.iter().flat_map(|capture| &capture.escaped_rows).collect();
    assert!(rows.iter().any(|row| row.contains("STARCOM_PRIMARY_READY")));
    assert!(rows.iter().any(|row| row.contains("STARCOM_ALTERNATE_READY")));
    drop(inspector);
    assert_eq!(before, tmux(&["list-panes", "-s", "-t", "starcom", "-F", "#{pane_id} #{pane_width} #{pane_height} #{pane_pid}"]));
    assert_eq!(environment, tmux(&["show-environment", "-t", "starcom", "STARCOM_TEST_ENV"]));
    let deadline = time::Instant::now() + time::Duration::from_secs(5);
    while !tmux(&["list-clients", "-F", "#{client_pid}"]).trim().is_empty() {
        assert!(time::Instant::now() < deadline, "inspection client did not detach");
...
fn trust_failures_precede_authentication_and_do_not_rewrite_known_hosts() {
    for (name, kind) in [("known_hosts.empty", ssh::Kind::UnknownHostKey),
        ...
        ("known_hosts.revoked", ssh::Kind::Configuration)]
    {
        ...
        let error = ssh::Connection::connect(&options).err().expect("unsafe connection succeeded");
...
fn missing_sessions_and_servers_are_not_created() {
    ...
    let result = inspect::Inspector::attach(&options(), &missing, absent_socket.to_str())
        .and_then(|mut inspector| inspector.observe(0));
    assert!(result.is_err());
    assert!(!absent_socket.exists(), "inspection created a new tmux server");
    assert_eq!(tmux(&["list-sessions", "-F", "#{session_name}"]).trim(), "starcom");
}

#[test]
#[ignore = "requires the isolated SSH/tmux fixture"]
fn stdout_and_stderr_remain_separate_without_a_pty() {
    let mut channel = ssh::Connection::connect(&options()).unwrap()
        .exec("printf output; printf diagnostic >&2; test ! -t 0").unwrap();
    let deadline = time::Instant::now() + time::Duration::from_secs(5);
    ...
    loop {
        ...
        match channel.read_stderr(&mut buffer) {
            Ok(count) => { stderr.extend_from_slice(&buffer[..count]); progress |= count != 0; }
            Err(error) if error.kind() == io::ErrorKind::WouldBlock => {},
            Err(error) => panic!("stderr: {error}"),
        }
        match io::Read::read(&mut channel, &mut buffer) {
            Ok(count) => { stdout.extend_from_slice(&buffer[..count]); progress |= count != 0; }
            Err(error) if error.kind() == io::ErrorKind::WouldBlock => {},
            Err(error) => panic!("stdout: {error}"),
        }
        if channel.eof() { break; }
        if !progress { channel.wait(deadline).unwrap(); }
    }
...
fn stalled_channel_has_a_deadline() {
    let mut channel = ssh::Connection::connect(&options()).unwrap().exec("exec sleep 2").unwrap();
...
fn inspection_cli_exercises_the_real_embedded_transport() {
    ...
    let result = process::Command::new(env!("CARGO_BIN_EXE_starcom-inspect"))
        .args(["--host", "127.0.0.1", "--user", &options.user, "--session", "starcom", "--port", &options.port.to_string()])
        .arg("--known-hosts").arg(&options.known_hosts)
        .arg("--identity").arg(root().join("id_ed25519"))
        .arg("--socket").arg(root().join("tmux.sock"))
        .args(["--history", "0", "--timeout", "5"]).output().unwrap();
